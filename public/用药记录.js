document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelector('#medicationTable thead th:last-child')?.remove();
  const records=[
  ...
  const render=()=>{
    ...
    $('#medicationTable tbody').innerHTML=data.map(record=>`<tr><td><b>${record.patientId}</b></td><td><b>${record.medicine}</b></td><td>${record.createdAt}</td><td class="frequency">${record.frequency}</td><td>${record.addedAt}</td></tr>`).join('');
